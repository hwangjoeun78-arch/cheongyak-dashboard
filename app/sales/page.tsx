...
import Dday, { calcDday } from '@/components/ui/Dday'
...
function SalesInner() {
  ...
  const [view, setView] = useState<'list' | 'card' | 'timeline'>('list')
  ...
  // 관심단지 필터 적용된 목록
  const displayItems = (data?.items ?? []).filter(item => !favOnly || has(item.HOUSE_MANAGE_NO))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 800, color: 'var(--ink)' }}>분양정보</h1>
        ...
        <div className="flex gap-2 items-center flex-wrap">
          ...
          <button onClick={() => setFavOnly(v => !v)}
            style={{
              padding: '7px 12px', fontSize: 13, fontWeight: 700, borderRadius: 'var(--r-sm)',
              color: favOnly ? '#fff' : 'var(--ink-2)',
              background: favOnly ? '#F5A623' : 'var(--bg-2)', transition: 'all .15s',
            }}>
            ★ 관심 {count > 0 && `(${count})`}
          </button>
          {/* 뷰 전환 세그먼트 */}
          <div style={{ display: 'inline-flex', background: 'var(--bg-2)', borderRadius: 'var(--r-sm)', padding: 3, gap: 2 }}>
            {([['list', '목록'], ['card', '카드'], ['timeline', '일정']] as const).map(([v, label]) => (
      ...
      {!isLoading && displayItems.length === 0 && (
        <div className="cy-panel text-center py-16" style={{ color: 'var(--ink-4)' }}>
          {favOnly ? '관심 등록한 단지가 없습니다. 별 아이콘을 눌러 추가하세요.' : '해당 조건의 분양 데이터가 없습니다.'}
        </div>
      )}

      {/* 카드형 뷰 */}
      {view === 'card' && displayItems.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {displayItems.map((item, i) => (
            <div key={i} onClick={() => setSelected(item)} className="cy-card cy-panel cursor-pointer" style={{ padding: 18 }}>
              <div className="flex items-start justify-between gap-1 mb-3">
                <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--ink)', lineHeight: 1.3 }}>{item.HOUSE_NM}</div>
                <FavStar active={has(item.HOUSE_MANAGE_NO)} onClick={() => toggle(item.HOUSE_MANAGE_NO)} />
              </div>
      ...
      {/* 일정형(타임라인) 뷰 — 마감일 순 정렬 */}
      {view === 'timeline' && displayItems.length > 0 && (
        <div className="cy-panel cy-panel-pad">
          <div style={{ position: 'relative', paddingLeft: 24 }}>
            <div style={{ position: 'absolute', left: 7, top: 8, bottom: 8, width: 2, background: 'var(--border-2)' }} />
            {[...displayItems]
              .sort((a, b) => (calcDday(a.RCEPT_ENDDE) ?? 9999) - (calcDday(b.RCEPT_ENDDE) ?? 9999))
              .map((item, i) => {
                const dd = calcDday(item.RCEPT_ENDDE)
                const dotColor = dd === null ? 'var(--border-3)' : dd < 0 ? 'var(--ink-4)' : dd <= 3 ? 'var(--amber)' : 'var(--primary)'
                return (
                  <div key={i} onClick={() => setSelected(item)} className="cy-row cursor-pointer"
                    style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 14, padding: '12px 10px', borderRadius: 'var(--r-sm)' }}>
                    <span style={{ position: 'absolute', left: -21, width: 12, height: 12, borderRadius: '50%', background: dotColor, border: '2px solid var(--surface)' }} />
                    <div style={{ width: 70, flexShrink: 0 }}><Dday dateStr={item.RCEPT_ENDDE} /></div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: 14.5, color: 'var(--ink)' }}>{item.HOUSE_NM}</div>
                      <div style={{ fontSize: 12.5, color: 'var(--ink-3)', marginTop: 2 }}>
                        {item.SUBSCRPT_AREA_CODE_NM} · 접수 {formatDate(item.RCEPT_BGNDE)} ~ {formatDate(item.RCEPT_ENDDE)}
                      </div>
                    </div>
                    <StatusBadge status={getStatus(item.RCEPT_BGNDE, item.RCEPT_ENDDE)} />
                    <FavStar active={has(item.HOUSE_MANAGE_NO)} onClick={() => toggle(item.HOUSE_MANAGE_NO)} />
              ...
              {isLoading
                ? Array.from({ length: 8 }).map((_, i) => (
                    <tr key={i} style={{ borderTop: '1px solid var(--border)' }}>
                      <td colSpan={8} className="px-4 py-3"><div className="h-4 animate-pulse rounded" style={{ background: 'var(--bg-2)' }} /></td>
                    </tr>
                  ))
                : displayItems.map((item, i) => (
                      <tr key={i} onClick={() => setSelected(item)} className="cy-row cursor-pointer" style={{ borderTop: '1px solid var(--border)' }}>
                        <td className="px-3 py-2 text-center">
                          <FavStar active={has(item.HOUSE_MANAGE_NO)} onClick={() => toggle(item.HOUSE_MANAGE_NO)} size={16} />
                        </td>
                        <td className="px-4 py-3 font-medium" style={{ color: 'var(--ink)' }}>{item.HOUSE_NM}</td>
      ...
      {totalPages > 1 && !favOnly && (
        <div className="flex justify-center gap-2">
          <button onClick={() => setPageNo(p => Math.max(1, p - 1))} disabled={pageNo === 1}
            className="px-3 py-1.5 rounded-md text-sm disabled:opacity-40" style={{ border: '1px solid var(--border-2)', color: 'var(--ink-2)' }}>이전</button>
          <span className="px-4 py-1.5 text-sm" style={{ color: 'var(--ink-2)' }}>{pageNo} / {totalPages}</span>
          <button onClick={() => setPageNo(p => Math.min(totalPages, p + 1))} disabled={pageNo === totalPages}
            className="px-3 py-1.5 rounded-md text-sm disabled:opacity-40" style={{ border: '1px solid var(--border-2)', color: 'var(--ink-2)' }}>다음</button>
        </div>
      )}
